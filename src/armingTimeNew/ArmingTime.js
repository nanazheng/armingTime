...
const ArmingTime = (props) => {
  const { value, form, data,  prevHour, prevMin, nextHour, nextMin, timelineIndex,
    timelinersLength, } = props;
  ...
  const disabledEndHours = () => {
    ...
    if(timelineIndex === timelinersLength - 1) {
      ...
      disabledHours.splice(parseInt(_startTime[0]), nextHour - parseInt(_startTime[0]) + 1)
    }
    return disabledHours
  }
