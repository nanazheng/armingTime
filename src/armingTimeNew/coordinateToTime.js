export function coordinateToTime(startOffset, endOffset) {
  ...
  const startHour = parseInt(startOffset * 2 / 60);
  const startMin = startOffset * 2 % 60;
  const endHour = parseInt(endOffset * 2 / 60);
  const endMin = endOffset * 2 % 60;
  const startTime = `${startHour > 9 ? startHour : `0${startHour}`}:${startMin > 9 ? startMin : `0${startMin}`}`
